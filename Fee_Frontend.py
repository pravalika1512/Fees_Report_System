    ...
    def __init__(self, master):
        ...
        def Insert():
            if (len(self.admsn.get()) != 0):
                ...
                p=self.paid.get()
                r=t-p
                Fee_Backend.insert(self.id.get(),self.recpt.get(), self.name.get(), self.admsn.get(), self.date.get(),
                                   self.branch.get(), self.sem.get(), self.total.get(), self.paid.get(),
                                   r)
                self.list.delete(0, END)
                self.list.insert(END, (self.id.get(),self.recpt.get(), self.name.get(), self.admsn.get(), self.date.get(),
                                       self.branch.get(), self.sem.get(), self.total.get(), self.paid.get(),
                                       r))
                self.due.set(r)
        ...
        # ==================================================Frames===================================================
        Main_Frame = Frame(self.master, bg='#E0F2F1')
        Main_Frame.grid()

        Title_Frame = LabelFrame(
            Main_Frame, width=1350, height=70, bg='#64B5F6', relief='groove',borderwidth=4)
        Title_Frame.pack(side=TOP)

        self.lblTitle = Label(Title_Frame, font=('verdana', 30, 'bold'), text='FEE REPORT',
                              bg='#64B5F6', padx=15)
        self.lblTitle.grid(padx=600)

        Data_Frame = Frame(Main_Frame, width=1350, height=350,
                           bg='#E0F2F1', relief='groove', bd=5)
        Data_Frame.pack(side=TOP, padx=15)

        Frame_1 = LabelFrame(Data_Frame, width=850, height=350, bg='#E0F2F1', relief='groove', bd=5,
                             text='Informations', font=('arial', 15, 'bold'))
        Frame_1.pack(side=LEFT, padx=10)

        Frame_2 = LabelFrame(Data_Frame, width=495, height=350, bg='#E0F2F1', relief='groove', bd=5,
                             text='Fee Receipt', font=('arial', 15, 'bold'))
        Frame_2.pack(side=RIGHT, padx=10)

        List_Frame = Frame(Main_Frame, width=1350, height=150,
                           bg='#E0F2F1', relief='ridge', bd=8)
        ...
        # ===================================================Labels================================================
        self.id_label = Label(Frame_1, text='PRN NO.. : ', font=(
            'arial', 14, 'bold'),bg='#E0F2F1')
        self.id_label.grid(row=0, column=0, padx=10, sticky=W)

        self.recpt_label = Label(Frame_1, text='RECEIPT NO. : ', font=(
            'arial', 14, 'bold'),bg='#E0F2F1')
        self.recpt_label.grid(row=1, column=0, padx=10, sticky=W)

        self.name_label = Label(Frame_1, text='STUDENT NAME : ', font=(
            'arial', 14, 'bold'),bg='#E0F2F1')
        self.name_label.grid(row=2, column=0, padx=10, sticky=W)

        self.admsn_label = Label(Frame_1, text='ADMISSION NO. : ', font=(
            'arial', 14, 'bold'),bg='#E0F2F1')
        self.admsn_label.grid(row=3, column=0, padx=10, sticky=W)

        self.Date_label = Label(Frame_1, text='DATE : ', font=(
            'arial', 14, 'bold'),bg='#E0F2F1')
        self.Date_label.grid(row=4, column=0, padx=10, sticky=W)

        self.branch_label = Label(Frame_1, text='BRANCH : ', font=(
            'arial', 14, 'bold'),bg='#E0F2F1')
        self.branch_label.grid(row=5, column=0, padx=10, sticky=W)

        self.sem_label = Label(Frame_1, text='SEMISTER : ', font=(
            'arial', 14, 'bold'),bg='#E0F2F1')
        self.sem_label.grid(row=1, column=2, padx=10, sticky=W)
        ...
        self.id_entry = Entry(Frame_1, font=(
            'arial', 14), textvariable=self.id)
        self.id_entry.grid(row=0, column=1, padx=10, pady=5)

        self.recpt_entry = Entry(Frame_1, font=(
            'arial', 14), textvariable=self.recpt)
        self.recpt_entry.grid(row=1, column=1, padx=10, pady=5)
        ...
        self.admsn_entry.grid(row=3, column=1, padx=10, pady=5)

        self.Date_entry = Entry(Frame_1, font=(
            'arial', 14), textvariable=self.date)
        self.Date_entry.grid(row=4, column=1, padx=10, pady=5)

        self.branch_entry = ttk.Combobox(Frame_1, values=(' ', 'CSE', 'IT','E&TC','EE','Civil','Mech','AIDS'),
                                         font=('arial', 14), width=19, textvariable=self.branch)
        self.branch_entry.grid(row=5, column=1, padx=10, pady=5)

        self.sem_entry = ttk.Combobox(Frame_1, values=(' ', 'FIRST', 'SECOND', 'THIRD','FOURTH','FIFTH','SIXTH','SEVENTH','EIGHT'), font=('arial', 14), width=19,
                                     textvariable=self.sem)
        self.sem_entry.grid(row=1, column=3, padx=10, pady=5)

        self.total_entry = Entry(Frame_1, font=(
            'arial', 14), width=10, textvariable=self.total)
        self.total_entry.grid(row=2, column=3, padx=10, pady=5)
        ...
        # ==================================================Frame_2=================================================
        self.Display = Text(Frame_2, width=45, height=12,
                            font=('arial', 14, 'bold'))
        ...
        self.list = Listbox(List_Frame, font=(
            'arial', 13, 'bold'), width=140, height=12)
